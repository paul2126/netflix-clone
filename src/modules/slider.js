export function initSliders() {
  ...
  if (rankingSlider) {
    rankingSlider.addEventListener('scroll', () => updatePageIndicator('ranking'));
  }
...
function slideItems(slider, sliderType, isNext) {
  ...
  if (sliderType === 'ranking') {
    itemSelector = '.ranking-item';
    itemWidth = slider.querySelector(itemSelector).offsetWidth + parseInt(getComputedStyle(slider).gap) || 283; // 259px width + 24px gap
  } else if (sliderType === 'cards') {
    itemSelector = '.card';
    itemWidth = slider.querySelector(itemSelector).offsetWidth + parseInt(getComputedStyle(slider).gap) || 259; // 240px width + 19px gap
  }
...
function handlePageDotClick(event) {
  ...
  if (sliderType === 'ranking') {
    slider = document.getElementById('ranking-slider');
    itemSelector = '.ranking-item';
    itemWidth = slider.querySelector(itemSelector).offsetWidth + parseInt(getComputedStyle(slider).gap) || 283;
  } else if (sliderType === 'cards') {
    slider = document.getElementById('card-slider');
    itemSelector = '.card';
    itemWidth = slider.querySelector(itemSelector).offsetWidth + parseInt(getComputedStyle(slider).gap) || 259;
  }
